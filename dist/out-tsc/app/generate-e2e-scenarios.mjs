...
function getTemplateUrl(cls) {
    ...
    if (decorator) {
        ...
        if (args.length > 0 && args[0].getKind() === SyntaxKind.ObjectLiteralExpression) {
            ...
            const templateUrlProp = properties.find((p) => p.getKind() === SyntaxKind.PropertyAssignment && p.getName() === 'templateUrl');
            if (templateUrlProp) {
...
function analyzeTemplate(template, className) {
    const scenarios = [];
    // Identify buttons
...
//# sourceMappingURL=generate-e2e-scenarios.mjs.map